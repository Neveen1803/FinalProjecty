...
@WebServlet(name="ViewBookedTicketToCancel",urlPatterns= {"/User/ViewBookedTicketToCancel"})
public class ViewBookedTicketToCancel extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		sb.setUser(user);
		List<SeeBooked> ViewTickets=sb.ViewTicketsToCancel();
		...
		for(int i=0;i<ViewTickets.size();i++) {
			...
			jobj.put("BookingId", ""+ViewTickets.get(i).getBooking().getBookingId());
			jsonArray.add(jobj);
		}
